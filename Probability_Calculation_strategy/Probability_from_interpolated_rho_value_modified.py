...
print(np.shape(m_cell_sph))

################# Load the saved interpolated rho values ##########################
interpolated_rho=pickle.load(open("./interpolated rho.p","rb"))
#interpolated_rho=interpolated_rho/0.9972217712148413 # Pavan's trick
print(np.shape(interpolated_rho))
###########################################################################

rad_2_deg=180.0/np.pi
deg_to_rad=np.pi/180.0

phi_angle_all=m_cell_sph[2,:]
...
while phi_value < (2.0*np.pi):
	...
	index_phi=np.asarray(np.where(phi_angle_all == phi_value))
	size_index_phi=np.shape(index_phi)
	theta_at_index=m_cell_sph[1,index_phi] # pick up the theta values
	####################### Sorting rho according to sorted index of theta ########################
	xvar=theta_at_index[0,:] 
    
    	rho_at_index=interpolated_rho[index_phi]
	yvar=rho_at_index[0,:]
    	rho_sorted_acc_to_theta=index_sorted(xvar,yvar) # calling the index_sorted function
    	rho_sorted_acc_to_theta=np.reshape(rho_sorted_acc_to_theta,(1,len(rho_sorted_acc_to_theta))) # reshaping into a single array
...
print('Covered cell = ' + str(total_size))
