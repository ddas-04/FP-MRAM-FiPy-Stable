...
def get_phi_tuple(number_of_cells):
    Filename = './with_axis_0_VTK_files/with_axis_0_img_00801.vtk' #This file contains the phi value
...
phi_value=0
phi_step=360.0
delta=(2*np.pi)/phi_step
phi_save=[]
theta_max=[]
integ_rho_sin_theta_hemisphere=[]
i=0
print('\nDefault calculation is done for lower hemisphere.\nIf you want to calculate the probability for upper hemisphere, Enter "u". \n')
...
while phi_value<=(2*np.pi):
    ...
    ####################### Calculation for hemisphere #################################
    if choice =='u':
	theta_sorted_hemisphere_index = np.asarray(np.where(theta_sorted<=0.5*np.pi))
    else:
	theta_sorted_hemisphere_index = np.asarray(np.where(theta_sorted>0.5*np.pi))
    
    theta_sorted_hemisphere = theta_sorted[theta_sorted_hemisphere_index]
    
    rho_sorted_acc_to_theta_hemisphere = rho_sorted_acc_to_theta[0,theta_sorted_hemisphere_index]

    theta_sorted_hemisphere=theta_sorted_hemisphere.flatten()
    rho_sorted_acc_to_theta_hemisphere=rho_sorted_acc_to_theta_hemisphere.flatten()
    print(max(theta_sorted_hemisphere)*(180.0/np.pi))
    rho_sin_theta_hemisphere=rho_sorted_acc_to_theta_hemisphere*np.sin(theta_sorted_hemisphere) # calculating rho*sin(theta) part
    integ_rho_sin_theta_for_phi_val_hemisphere=trapz_irregular(theta_sorted_hemisphere,rho_sin_theta_hemisphere) #integration for irregular grid size
    integ_rho_sin_theta_hemisphere=np.append(integ_rho_sin_theta_hemisphere,integ_rho_sin_theta_for_phi_val_hemisphere) 
    
    phi_value=phi_value+delta
    

total_Probability=trapz_irregular(phi_save,integ_rho_sin_theta_hemisphere)

if choice == 'u':
	print('Probability at upper hemisphere = ' + str(total_Probability))
else:
	print('Probability at lower hemisphere = ' + str(total_Probability))
